...
namespace Tests
{
    ...
    struct SimpleTest
    {
        ...
        {
            this->test_empty();
            this->test_set_insert_erase_3200();
            this->test_set_insert_erase_3200_descending();
            this->test_map_insert_erase_3200();
            this->test_map_insert_erase_3200_descending();
            this->test2_map_insert_erase_strings();
            this->test_set_100000_uint64();
            this->test_multiset_100000_uint32();
        }
        ...
        void test2_map_insert_erase_strings()
        {
            ...
            for (unsigned int b = 0; b < letters.size(); ++b)
            {
                for (unsigned int a = 0; a < letters.size(); ++a)
                {
                    ...
                    REQUIRE(bt.find(key)->second == a * letters.size() + b);
                    REQUIRE(bt.erase_one(key));
                }
            }
        ...
        void test_set_100000_uint64()
        {
            ...
            for (uint64_t i = 10; i < 100000; ++i)
            {
                ...
                {
                    bt.insert(std::make_pair(key, (uint8_t)(key % 100)));
                }
            }

            REQUIRE(bt.size() == 1000);
        }
        ...
        void test_multiset_100000_uint32()
        {
            ...
            REQUIRE(bt.size() == 100000);
        }
    };

    TEST_CASE("Testing binary search for different slot sizes", "[SimpleTest]")
    {
        std::cout << "Running test in [SimpleTest]" << std::endl;
